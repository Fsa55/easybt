...
class Backtest():
    ...
    """
    def __init__(self, data, cost_per_trade = 0.0, multiplier = 1.):
        """
        Parameters:
        -----------
    ...
    def create_trades_list(self):
        tradesbuy = pd.DataFrame(data = self.data.buy.dropna().values,index = self.data.buy.dropna().index,columns = ['price'])
        tradesbuy['type'] = 'BUY'
        tradessell = pd.DataFrame(data = self.data.sell.dropna().values,index = self.data.sell.dropna().index,columns = ['price'])
        tradessell['type'] = 'SELL'
        tradesexit_sell = pd.DataFrame(data = self.data.exit_sell.dropna().values,index = self.data.exit_sell.dropna().index,columns = ['price'])
        tradesexit_sell['type'] = 'EXIT_SELL' 
        tradesexit_buy = pd.DataFrame(data = self.data.exit_buy.dropna().values,index = self.data.exit_buy.dropna().index,columns = ['price'])
    ...
    def calculate_maxDD(self):
        ...
        self.data = self.data.join(pd.DataFrame(self.data.pnl[j:i].values,index = self.data.index[j:i], columns=['MAX_DD']),how = 'outer')
    ...
    def run(self):
        ...
        self.create_trades_list()
        self.calculate_pnl()
        self.calculate_maxDD()
    ...
    def plot(self,title = 'Backtest',sub_ind = [],height = 700,render = None):
        ...
        """
        self.backtest_plot(df = self.data.drop(['buy_signal','sell_signal','exit_buy_signal','exit_sell_signal'],axis = 1),
    ...
    def backtest_plot(self,title,df,pnl = True, bars = True, sub_ind = [],height = 700,render = None):
        ...
        fig = make_subplots(rows = len(sub_ind) + 1, cols=1,
                            row_heights= [0.4]+[0.6/len(sub_ind) for item  in sub_ind],
                            shared_xaxes=True, 
        ...
        for i in range(len(sub_ind)):
            ...
            for sub in sub_ind[i]:
                ...
                del df1[sub]
        if bars == True:        
            fig.add_trace(go.Candlestick(x = df1.index, open = df1['open'], high = df1.high, low = df1.low, close = df1.close,name = 'bars'), row = 1,col = 1)
        else:
            fig.add_trace(go.Scatter(x=df1.index, y=df1.close.values, name= 'close'),row = 1, col =1)
        fig.add_trace(go.Scatter(x = df1.index, y=df.buy.values, mode='markers',marker=dict(color='rgb(12, 255, 12)',size=8,line=dict(color='rgb(0, 0, 0)',width=1)),name = 'Buy'), row = 1,col = 1)
        fig.add_trace(go.Scatter(x = df1.index, y=df.sell.values, mode='markers', marker=dict(color='rgb(252, 12, 12)',size=8,line=dict(color='rgb(0, 0, 0)',width=1)),name = 'Sell'), row = 1,col = 1)
        fig.add_trace(go.Scatter(x = df1.index, y=df.exit_sell.values, mode='markers',marker_color='rgb(0, 0, 0)',marker_size= 8,name = 'exit_sell'), row = 1,col = 1)
        fig.add_trace(go.Scatter(x = df1.index, y=df.exit_buy.values, mode='markers',marker_color='rgb(0, 0, 0)',marker_size= 8,name = 'exit_buy'), row = 1,col = 1)
        del df1['open'],df1['high'],df1['low'],df1['close'],df1['buy'],df1['sell'],df1['exit_buy'],df1['exit_sell']
